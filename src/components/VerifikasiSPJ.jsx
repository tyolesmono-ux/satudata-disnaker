              ...
              <tbody className="divide-y divide-gray-100">
                {activeSPJs.map((spj, i) => {
                  ...
                  <tr key={i} className="hover:bg-blue-50/40 transition-colors">
                    ...
                    <td className="p-4 font-bold text-gray-800">{formatRupiah(spj.total_kotor)}</td>
                    <td className="p-4">
                      {Number(spj.total_ppn) > 0 && <div className="text-xs flex justify-between w-28"><span className="text-gray-500">PPN:</span><span className="font-semibold">{formatRupiah(spj.total_ppn).replace('Rp','')}</span></div>}
                      {Number(spj.total_pph21) > 0 && <div className="text-xs flex justify-between w-28"><span className="text-gray-500">PPh 21:</span><span className="font-semibold">{formatRupiah(spj.total_pph21).replace('Rp','')}</span></div>}
                      {Number(spj.total_pph22) > 0 && <div className="text-xs flex justify-between w-28"><span className="text-gray-500">PPh 22:</span><span className="font-semibold">{formatRupiah(spj.total_pph22).replace('Rp','')}</span></div>}
                      {Number(spj.total_pph23) > 0 && <div className="text-xs flex justify-between w-28"><span className="text-gray-500">PPh 23:</span><span className="font-semibold">{formatRupiah(spj.total_pph23).replace('Rp','')}</span></div>}
                      {Number(spj.total_ppn) === 0 && Number(spj.total_pph21) === 0 && Number(spj.total_pph22) === 0 && Number(spj.total_pph23) === 0 && <span className="text-gray-400 italic text-xs">Nihil</span>}
                    </td>
                    <td className="p-4">
                ...
                <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg mb-6 grid grid-cols-2 gap-4">
                  ...
                  <div>
                     ...
                     <div className="px-4 py-2 bg-white rounded border text-sm font-bold text-gray-700">
                       {selectedSPJ.bidang} - {formatRupiah(selectedSPJ.total_kotor)}
                     </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              ...
              <div className="space-y-4 bg-gray-50 p-5 rounded-xl border">
                <SelectField label="Pengguna Anggaran (PA)" value={pejabat.pa_nip} onChange={e => setPejabat({ ...pejabat, pa_nip: e.target.value })}><option value="">-- Pilih Pejabat --</option>{pegawaiASN.map(p => <option key={p.nip} value={p.nip}>{p.nama}</option>)}</SelectField>
                <SelectField label="Bendahara Pengeluaran" value={pejabat.bendahara_nip} onChange={e => setPejabat({ ...pejabat, bendahara_nip: e.target.value })}><option value="">-- Pilih Pejabat --</option>{pegawaiASN.map(p => <option key={p.nip} value={p.nip}>{p.nama}</option>)}</SelectField>
                <SelectField label="Pejabat Pelaksana Teknis (PPTK)" value={pejabat.pptk_nip} onChange={e => setPejabat({ ...pejabat, pptk_nip: e.target.value })}><option value="">-- Pilih Pejabat --</option>{pegawaiASN.map(p => <option key={p.nip} value={p.nip}>{p.nama}</option>)}</SelectField>
                <SelectField label="Sekretaris selaku PPK" value={pejabat.ppk_nip} onChange={e => setPejabat({ ...pejabat, ppk_nip: e.target.value })}><option value="">-- Pilih Pejabat --</option>{pegawaiASN.map(p => <option key={p.nip} value={p.nip}>{p.nama}</option>)}</SelectField>
              </div>
              ...
              <div className="mt-8 flex justify-end gap-3 pt-4 border-t">
                ...
                <button onClick={proceedToPrint} className="flex items-center px-8 py-2.5 bg-[#0A192F] text-white rounded-lg font-bold shadow-lg hover:bg-[#122442] hover:-translate-y-0.5 transition-all">
                   <Printer size={16} className="mr-2 text-[#D4AF37]" /> Tampilkan Kertas Kerja
                </button>
              </div>
            </div>
          </div>
